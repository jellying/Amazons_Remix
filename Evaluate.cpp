...
double Eval::CalMob(MapType map, int threadNum)
{
	...
	//			{
	//				m1 += (1.0*THfreeval[threadNum][nextwhite.x][nextwhite.y] / pow(2, kingstep));
	//			}
	//			kingstep++;
	//			nextwhite.x = nextwhite.x + dir[j][0];
	//			nextwhite.y = nextwhite.y + dir[j][1];
	//		}
	//		kingstep = 0;
	//		while (InBoard(nextred.x, nextred.y) && map[nextred.x][nextred.y] == BLANK)
	//		{
	//			if (QueenMove[thread][nextred.x][nextred.y][0] + QueenMove[thread][nextred.x][nextred.y][1] < 1000)
	//			{
	//				m2 += (1.0*THfreeval[thread][nextred.x][nextred.y] / pow(2, kingstep));
	//			}
	//			kingstep++;
	//			nextred.x = nextred.x + dir[j][0];
	//			nextred.y = nextred.y + dir[j][1];
	//		}
	//	}
	//	if (m1 < 1)
	//	{
